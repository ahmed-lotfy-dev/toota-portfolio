        ...
        <form wire:submit.prevent="subscribe" class="space-y-4">
            <div class="relative">
                <input type="email" 
                       ...
                       class="w-full px-5 py-4 bg-stone-800 border border-transparent text-[#FDFCF8] placeholder-stone-600 focus:outline-none focus:bg-stone-700 focus:border-stone-500 transition-all duration-300 @error('email') @enderror">
                
                @error('email')
                    <p class="-bottom-6 left-0 text-red-400 text-xs my-3">{{ $message }}</p>
                @enderror
            </div>
