<div x-data="{ sidebarOpen: false }" class="flex h-screen overflow-hidden bg-stone-900">
    <!-- Sidebar -->
    <aside :class="sidebarOpen ? 'w-64' : 'w-20 md:w-64'"
        class="fixed inset-y-0 left-0 z-50 bg-stone-900 border-r border-stone-800 text-stone-200 transition-all duration-300 ease-in-out md:static shrink-0">

        <!-- Header -->
        <div class="flex items-center h-16 px-4 md:px-6 border-b border-stone-800"
            :class="sidebarOpen ? 'justify-between' : 'justify-center md:justify-between'">

            <!-- Dashboard Text (Desktop Only) -->
            <a href="{{ url('/') }}"
                class="text-2xl font-bold tracking-wide text-white hover:opacity-80 transition-opacity whitespace-nowrap overflow-hidden hidden md:block">
                Dashboard
            </a>

            <!-- Mobile Menu Toggle (Mobile Only) -->
            <button @click="sidebarOpen = !sidebarOpen"
                class="flex items-center justify-start w-full px-2 text-sm font-medium rounded-md group text-stone-400 hover:bg-stone-800 hover:text-white focus:outline-none md:hidden">
        ...
        <nav class="flex-1 px-4 py-4 space-y-1 overflow-y-auto">
            ...
            <div class="px-2 mb-2 text-xs font-medium uppercase text-stone-500"
                :class="sidebarOpen ? 'block' : 'hidden md:block'">Content</div>
