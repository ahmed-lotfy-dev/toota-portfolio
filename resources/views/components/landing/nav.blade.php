<nav class="fixed w-full z-50 bg-[#FDFCF8]/90 backdrop-blur-sm border-b border-stone-100 transition-all duration-300" x-data="{ open: false }">
    <div class="max-w-7xl mx-auto px-6 md:px-12">
        <div class="flex justify-between items-center h-24">
            
            <div class="shrink-0">
                <a href="/" class="flex items-center gap-3 group">
                    <div class="w-10 h-10 border border-stone-900 flex items-center justify-center bg-transparent group-hover:bg-stone-900 transition-colors duration-500">
                        <svg class="w-5 h-5 text-stone-900 group-hover:text-[#FDFCF8] transition-colors duration-500" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5">
                    ...
                    <span class="text-2xl font-bold text-stone-900 tracking-tight" style="font-family: 'Playfair Display', serif;">
                        {{ __('nav.brand') }}
                    </span>
                </a>
            </div>
            ...
            <div class="hidden md:flex items-center gap-8 rtl:space-x-reverse">
                <a href="/"
                   class="text-xs uppercase tracking-[0.15em] font-medium text-stone-500 hover:text-stone-900 transition-colors duration-300 relative group">
                    {{ __('nav.home') }}
                    <span class="absolute -bottom-2 left-1/2 -translate-x-1/2 w-0 h-px bg-stone-900 transition-all duration-300 group-hover:w-full"></span>
                </a>
            ...
            <div class="md:hidden flex items-center gap-4">
                 <a href="{{ url('/lang?lang=' . (session('locale') == 'en' ? 'ar' : 'en')) }}" class="text-xs font-bold text-stone-900 uppercase">
                    {{ session('locale') == 'en' ? 'AR' : 'EN' }}
                </a>

                <button @click="open = !open" type="button" class="text-stone-900 hover:text-stone-600 focus:outline-none">
                    <svg class="h-8 w-8" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                        <path x-show="!open" stroke-linecap="round" stroke-linejoin="round" d="M3.75 9h16.5m-16.5 6.75h16.5" /> <path x-show="open" stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" />
                    </svg>
                </button>
            </div>
        </div>
    ...
    <div x-show="open" 
        ...
        <div class="px-8 py-8 space-y-6 flex flex-col items-center text-center">
            ...
            <a href="/dashboard" class="text-sm uppercase tracking-widest text-stone-400 hover:text-stone-900 transition pt-4">
                {{ __('nav.dashboard') }}
            </a>
            <a href="/contact" class="w-full max-w-xs py-4 bg-stone-900 text-[#FDFCF8] uppercase tracking-widest text-sm hover:bg-stone-700 transition">
                {{ __('nav.contact') }}
            </a>
        </div>
    </div>
